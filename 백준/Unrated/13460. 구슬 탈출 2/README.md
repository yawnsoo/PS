...
문제 이해, 코드 구상은 어렵지 않았으나, 코드 구현 능력과 세세한/숨겨진 조건을 맞춰가는 부분(디버깅)에서 시간이 오래 걸림.
문제에서 주어진 조건들을 더 꼼꼼히 확인하고 코드 구상 및 구현으로 넘어가는 것이 더 나아보임.
코드 구현에서 실수도 많음. 컴퓨팅적인 사고도 더 필요.
